...
from create_roc import create_roc_graph_OvR, create_roc_graph_OvO 
...
def best_model_search_df(X, y, file, seed, cv):
    ...
    selector_random = RandomizedSearchCV(estimator=selector, param_distributions=grid, refit='roc_auc', scoring = ['accuracy', 'precision', 'recall', 'f1', 'roc_auc'],
...
def best_model_search_rf(X, y, file, seed, cv):
    ...
    # search across 100 different combinations, and use all available cores
    selector_random = RandomizedSearchCV(estimator=selector, param_distributions=grid, refit='roc_auc', scoring = ['accuracy', 'precision', 'recall', 'f1', 'roc_auc'],
                                   n_iter=100, cv=cv, verbose=2, random_state=seed, n_jobs=-2)
    # Fit the random search model
    selector_random.fit(X, y)
...
def compare_base_vs_best_model(name, path_prefix, feats, class_counts=None):
    ...
    thres_step = 4
    df.steps = df.steps.apply(lambda x: thres_step+1 if x > thres_step else x)

    #train_ind, test_ind = train_test_split(df.index, random_state=seed, stratify=df.steps, test_size=0.15)
    #df.iloc[test_ind].to_csv(f"output/test_{name}.csv", index=False)
    #df = df.iloc[train_ind].reset_index().drop("index", axis=1)

    # Transform into binary task. 1 for 1 num of steps, 0 for more steps
    if df_type == "f":
      df.steps = df.steps.apply(lambda x: 0 if x > df_step else x)
      offset = None
    else:
      df = df[df.steps > df_step]
      df.steps = df.steps.apply(lambda x: 0 if x > thres_step else x)
      offset = df_step

    print("UN", df.steps.unique())
    ...
    if "balanced" in name:
      ...
      #X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=seed, stratify=y, test_size=0.1)
      #pd.concat([X_test, y_test], axis=1).to_csv(f"output/test_{name}")
      X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=seed, stratify=y)

    # process X-train with k-fold, and evaluate on y_test

    print("TR SHAPE", X_train.shape, y_train.shape, "TE SHAPE", X_test.shape, y_test.shape)
    ...
    auc_ovr = create_roc_graph_OvR(X_test, y_test, best_mod, "output", f"{model_name}_{name}_ROC_OvR.png", f"{model_name}_{name}_ROC_AUC_OvR.csv", offset)
